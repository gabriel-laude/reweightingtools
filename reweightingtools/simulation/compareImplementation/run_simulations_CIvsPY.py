...
from reweightingtools.potential.MBP import mmMueller, LinearBias, pyMueller
from reweightingtools.potential.bias2D import Linear
...
perturbation = LinearBias(strength=5)
...
linear_2dx = Linear(strength=5)
mb=pyMueller(bias=linear_2dx)
...
bias_force_py = -linear_2dx.gradient(qABOBA[:,0], qABOBA[:,1]).swapaxes(0,1)
bias_py = linear_2dx.potential(qABOBA[:,0], qABOBA[:,1]).swapaxes(0,1)
