...
@author: schaefej51
openmm simulation script for a unbiased simulation at OpenMM Mueller Brown Potential custom force 
"""
from MBP import *
...
analyticforce = MullerForce()
if biased:
    perturbation = MullerBias()
else:
    perturbation = None
...
if equilibration:
    for i in range(equisteps):
        ...
        T_equi = 2*Ekin/(3*6.02214076e23*1.380649e-23 *0.001)
        # logging    
        log = open(output_directory + "temp_equi.txt", 'a')
        log.write(str(T_equi) + " \n")
        log.close();    
        integrator.step(1)
